...
function ode() {
	...
	k0 = parseFloat(window.document.getElementById("idk0").value);
	mu = parseFloat(window.document.getElementById("imu").value);
	Tob = parseFloat(window.document.getElementById("idto").value);
	an = 100, ant = 10, am = 5;
...
g = function (y, t) {
	...
	if ((tau * psy) != 0) {
		v[0] = y[1];
		kf = k0 + mu * y[0];
		v[1] = -(tau + psy) / (tau * psy) * y[1] - 1 / (tau * psy) * y[0] + kf * 1 / tau * u;
	}
	else {
		kf = k0 + mu * y[0];
		v[0] = -1 / (tau + psy) * y[0] + kf * psy * 1 / (tau + psy) * u;
	}
	return v;
}
function data() {
